...
ggpairs(data=(C_GWASReady %>% select(matches("PSP3") & !matches(".NMC") & !matches("R_starch") & !matches("Moisture"))), lower = list(continuous = wrap("smooth", method = "lm", color = "gray60", size = 1, alpha = 0.3))) + theme_bw() + theme(text = element_text(size = 10), strip.text.y = element_text(angle = 0), strip.text.x = element_text(angle = 0), axis.text.x = element_text(angle = 90))
#Save plot
# Create and save the plot directly
ggsave(
